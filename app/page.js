...
export default function Component() {
  ...
  if (session) {
    return (
      <div className=" flex">
        <Sidebar />
        <div className=" flex flex-col my-20 gap-4 items-center justify-center mx-auto bg-linear-to-r from-cyan-500 to-blue-500">
          <h2 className=" text-center text-5xl texor">
            <span className=" italic">Hello, </span>
            <span className=" italic text-orange-600 font-semibold hover:text-purple-700 hover:shadow-2xl rounded-lg hover:p-4">
              {session.user.name}
            </span>
          </h2>
          <p className=" text-3xl mt-5 p-4">
            Welcome ! Order and enjoy your favorite <span className=" text-orange-600">pizza</span> !!!
          </p>
        </div>
      </div>
    );
